function Select-Plugins {
  ...
  .NOTES
    ...
    2.2 - Kevin Kirkpatrick
      ...
      [X] Move comments into proper comment based help block so it can be reviewed via standard PS help system (EX: C:\PS>.\Get-Help Select-Plugins.ps1 -ShowWindow)
  ...
  <# old code, prior to module reformat
  ...
  # set the plugin path; if it can't be found, error out
  $PluginPath = "$PSScriptRoot\..\Plugins"
  if (-not(Test-Path $PluginPath)){

    throw "[$($PSCmdlet.MyInvocation.MyCommand.Name)][ERROR] Could not find 'Plugins' directory. $_"

  } # end if

  $pluginNameHash = @{Label="Plugin";expression={$_.Name -replace '(.*)\.ps1(?:\.disabled|)$', '$1'}}
  $pluginEnableHash = @{Label="Enabled";expression={$_.Name -notmatch '.*\.disabled$'}}

  $plugins = Get-ChildItem -Path $PluginPath -Include *.ps1, *.ps1.disabled -Recurse |
  Sort-Object {[int]($_.Name -replace '\D')} |
  Select-Object FullName, Name, $pluginNameHash, $pluginEnableHash
